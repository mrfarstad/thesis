...
__global__ void gpu_stencil_coop(float* __restrict__ d_u1,
			      float* __restrict__ d_u2)
{
    unsigned int i, j, k, q, x, y, z, iw, jw, kw,
                 xskip, yskip, zskip, 
                 xwork, ywork, zwork, 
                 idx;
    //unsigned int s;
        ...
        for (z=0; z<zwork; z++) {
            for (y=0; y<ywork; y++) {
                for (x=0; x<zwork; x++) {
                    ...
                    //kw=k*zwork+z;
                    kw=0;
                    idx = iw + jw*NX + kw*NX*NY;
                    if (iw>=STENCIL_DEPTH && iw<NX-STENCIL_DEPTH &&
                        jw>=STENCIL_DEPTH && jw<NY-STENCIL_DEPTH &&
                        kw>=STENCIL_DEPTH && kw<NZ-STENCIL_DEPTH)
                        d_u2[idx] = stencil(d_u1, idx);
                }
            }
        }
...
__global__ void gpu_stencil_coop_backup(float* __restrict__ d_u1,
			      float* __restrict__ d_u2)
{
