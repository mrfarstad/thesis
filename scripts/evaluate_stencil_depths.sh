...
if [[ $# -lt 7 ]] ; then
    echo 'arg: (base/smem/coop/coop_smem) NGPUS DIM BLOCK_X BLOCK_Y BLOCK_Z STENCIL_DEPTH'
    exit 0
fi

project_folder=$(echo ${PWD} | sed 's/thesis.*/thesis/')
config=$project_folder/configs/yme/wide_stencil.conf
source $project_folder/constants.sh # Required for $REPEAT
...
sed -i -re 's/(STENCIL_DEPTH = )[0-9]+/\1'$7'/' $config
