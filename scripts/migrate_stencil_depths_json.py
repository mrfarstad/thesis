                ...
                for version, version_db in domain_dim_db.items():
                    ...
                    for stencil_depth, stencil_depth_db in version_db.items():
                        ...
                        for iteration, iteration_db in stencil_depth_db.items():
                            ...
                            if entry_not_exists(new_db, [dimension, domain_dim, version, stencil_depth, iteration, host, config]):
                                new_db[dimension][domain_dim][version][stencil_depth][iteration][host][config] = {}
                            for metric, times in iteration_db[host][config].items():
                                if not "profile" in results_json and metric != "time":
                                    continue
                                if entry_not_exists(new_db, [dimension, domain_dim, version, stencil_depth, iteration, host, config, metric]):
                                    new_db[dimension][domain_dim][version][stencil_depth][iteration][host][config][metric] = {}
                                new_db[dimension][domain_dim][version][stencil_depth][iteration][host][config][metric] = iteration_db[host][config][metric]

copy("results/results_profile.json",                  "heid", "heuristic")
