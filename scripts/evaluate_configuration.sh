...
if [[ $# -lt 8 ]] ; then
    echo 'arg: (base/smem/coop/coop_smem) NGPUS DIM BLOCK_X BLOCK_Y BLOCK_Z STENCIL_DEPTH REPEAT UNROLL_X'
    exit 0
fi
...
[ ! -z "$9" ] && sed -i -re 's/(UNROLL_X=)[0-9]+/\1'$9'/' $constants
...
sed -i -re 's/(repeat = )[0-9]+,/\1'$8',/'    $config
