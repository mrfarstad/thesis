...
class Optimisation:
    ...
    def __optimise(self, vt, presets):
        ...
        topLevelVarVals = [
            [(var, val) for val in self.__possValues[var]] for var in vt.vars
        ]
        ...
        # Keep BLOCK_X >= BLOCK_Y, as the optimal configuration "always" follows this pattern
        topLevelTests = [
            d
            for d in topLevelTests
        ...
        if (
            vt.subtrees != []
        ):  # Then vt.subtrees is nonempty and so vt is a branch node.
            ...
            for valuation in topLevelTests:
                ...
                valuation.update(
                    self.__restrictArb(vt.flattenchildren(), self.__possValues)
                )
            ...
            possValuations = filter(
                (lambda v: (v is not None) and (self.__evaluator.score(v) is not None)),
                possValuations,
            )
            if possValuations == []:
            ...
            topLevelTests = filter(
                lambda v: self.__evaluator.score(v) is not None, topLevelTests
            )
            if topLevelTests == []:
                return None  # There are no tests which evaluated correctly.
